...
STACK_NAME=${APP_NAME}-${SERVICE_NAME}-datastore-${APP_ENVIRONMENT}
ARTEFACT_DIR="${PROJECT_ROOT}/output"
COGNITO_AUTH_ROLE=$(aws cloudformation describe-stacks --stack-name ${AUTH_STACK} --query 'Stacks[].Outputs[?OutputKey==`CognitoAuthenticatedRole`].OutputValue' --output text)
...
aws cloudformation deploy \
    ...
    --template-file ${ARTEFACT_DIR}/cfn/datastore.yml \
    --parameter-overrides \
        Environment=${APP_ENVIRONMENT} \
        DevOpsAdminRole=${ADMIN_ROLE} \
        CognitoAuthenticatedRole=${COGNITO_AUTH_ROLE} \
